...
//#define EDITABLE_HIERARCHY_FOLDER_TRANSFORM
...
namespace Sisus.HierarchyFolders
{
	public static class HierarchyFolderUtility
	{
		...
		public static void ResetTransformStateWithoutAffectingChildren(Transform transform, bool undoable = false)
		{
			...
			if(!gameObject.activeSelf)
			{
				...
				if(preferences.askAboutAllowInactiveHierarchyFolders)
				{
					...
					{
						preferences.activeToggle = HierarchyFolderPreferences.ActiveToggle.AffectsHierarchyFolder;
					}
					else
					{
						preferences.activeToggle = HierarchyFolderPreferences.ActiveToggle.AffectsChildren;
					}
				...
				if(preferences.activeToggle is not HierarchyFolderPreferences.ActiveToggle.AffectsHierarchyFolder)
				{
					Undo.RegisterFullObjectHierarchyUndo(gameObject, "Toggle Hierarchy Folder Children Active");

					gameObject.SetActive(true);

					if(preferences.activeToggle is HierarchyFolderPreferences.ActiveToggle.AffectsChildren && transform.childCount > 0)
					{
			...
			if(transform.localPosition != Vector3.zero || transform.localEulerAngles != Vector3.zero || transform.localScale != Vector3.one || (rectTransform && (rectTransform.anchorMin != Vector2.zero || rectTransform.anchorMax != Vector2.one || rectTransform.pivot != new Vector2(0.5f, 0.5f) || rectTransform.offsetMin != Vector2.zero || rectTransform.offsetMax != Vector2.zero)))
			{
				if(undoable)
				{
		...
		[MethodImpl(AggressiveInlining)]
		public static HideFlags GetHierarchyFolderTransformHideFlags(Transform transform) => GetHierarchyFolderTransformHideFlags(transform.gameObject.IsPrefabAssetOrInstance());

		[MethodImpl(AggressiveInlining)]
		public static HideFlags GetHierarchyFolderTransformHideFlags(bool isPrefabAssetOrInstance)
			#if EDITABLE_HIERARCHY_FOLDER_TRANSFORM
			=> HideFlags.None;
			#elif SHOW_HIERARCHY_FOLDER_TRANSFORM
			=> HideFlags.NotEditable;
			#else
			// Don't hide transform in prefabs or prefab instances to avoid internal Unity exceptions
			=> isPrefabAssetOrInstance ? HideFlags.NotEditable : HideFlags.HideInInspector | HideFlags.NotEditable;
			#endif
	}
}
#endif